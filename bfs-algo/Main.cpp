...
public:
    ...
    void to_prev_intersection()
    {
        ...
        while (q_r - q_f + 1 > 0)
        {
            ...
            // Checking all the connected nodes to the current node, and adding them to queue
    ...
    void bfs(){
        log("Starting bfs to destination");
        show_path = true;
        intersection_stack[ISLen][0] = dest[0];
        intersection_stack[ISLen][1] = dest[1];
        ISLen++;
        to_prev_intersection();
...
int main(int argc, char *argv[])
{
    Maze maze;
    maze.DFS_explore();
    log("Exploration is done, moving ahead with bfs.");
    maze.bfs();
}
